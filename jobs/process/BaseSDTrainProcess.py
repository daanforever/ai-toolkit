...
from jobs.process.BatchProcessor import GeneralBatchProcessor
...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def process_general_training_batch(self, batch: 'DataLoaderBatchDTO'):
        if self._batch_processor is None:
            self._batch_processor = GeneralBatchProcessor(self)
    ...
    def run(self):
        ...
        if self.datasets is not None:
            self.data_loader = get_dataloader_from_datasets(self.datasets, self.train_config.batch_size, self.sd, train_config=self.train_config)
        ...
        for step in range(start_step_num, self.train_config.steps):
            ...
            with torch.no_grad():
                ...
                dir_consistency_mean = 0.0  # Mean directional consistency per group (Adafactor)
