"""Run one LoRA experiment probe in a fresh Python subprocess."""
...
_ENV_FLAG = "ZIMAGE_EXPERIMENT_PROBE_SUBPROCESS"
_ENV_CONFIG = "ZIMAGE_EXPERIMENT_PROBE_CONFIG"
_ENV_STEP_TIMEOUT = "ZIMAGE_EXPERIMENT_STEP_TIMEOUT_S"
_SCRATCH_YAML = "_probe_job.yaml"
_STEP_S_RE = re.compile(
    r"^experiment: step=(\d+) step_s=([0-9.]+)\s*$", re.MULTILINE
)
...
def _repo_root() -> Path:
    # experiments/probe.py → z_image_diffsynth → diffusion_models → extensions_built_in → repo
...
def _step_timeout_s_from_env() -> float:
    raw = os.environ.get(_ENV_STEP_TIMEOUT, "30.0").strip()
    try:
        return float(raw)
    except ValueError:
        return 30.0
...
def _ensure_cuda_params(proc) -> None:
    ...
    for n, p in module.named_parameters():
        ...
        if pt != "cuda":
            break
    print(
        f"\nexperiment: cuda_mem_gb={mem_gb:.2f} gpu={torch.cuda.get_device_name(0)} "
    ...
    if reason:
        print(f"experiment: {reason}", flush=True)
        sys.exit(1)
...
def _enforce_runtime_gates(job_path: str) -> None:
    ...
    print(
        f"experiment: gpu={torch.cuda.get_device_name(0)} torch_cuda={torch.version.cuda}",
        flush=True,
    )
...
def _install_step_timer() -> None:
    ...
    if getattr(orig, "_zimage_exp_step_timer", False):
        return
    ...
    def timed(self, *args, **kwargs):
        ...
        if last is None:
            ...
            return
        dt = now - last
        print(f"\nexperiment: step={int(self.step_num)} step_s={dt:.3f}", flush=True)
        if step_timeout_exceeded(
            ...
            print(
                f"\nexperiment: step_timeout step={int(self.step_num)} "
                f"step_s={dt:.3f} limit={limit_s:.3f}",
                flush=True,
            )
            sys.exit(1)

    timed._zimage_exp_step_timer = True  # type: ignore[attr-defined]
...
def _run_child(
    ...
    try:
        ...
        timed_out = True
        msg = "experiment: child_wall_timeout\n"
...
def _load_session_mod():
    """File-path import of sibling session.py (script-safe; avoids parent DiT init)."""
    name = "_zids_exp_session"
    if name in sys.modules:
        return sys.modules[name]
    path = Path(__file__).resolve().parent / "session.py"
...
def _probe_subprocess_worker() -> None:
    ...
    _install_step_timer()
    _load_session_mod().install_session_hook()
