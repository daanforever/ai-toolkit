...
- `turbo_slot_weighting` present and not `dsigma` — omit the key (no A/B). dsigma is the base for content/style; `content` flips it; `balanced` is uniform over slots; `style` keeps last-heavy
...
process:
  - type: z_image_diffsynth_trainer
    train:
      ...
      # content = reverse dsigma (composition); style = dsigma (last-heavy); balanced = uniform over slots
