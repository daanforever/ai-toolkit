...
export async function PATCH(
  ...
  let body: { max_lr?: number; gaussian_mean?: number; gaussian_std?: number; weight_decay?: number; content_or_style?: string; timestep_type?: string };
  try {
  ...
  if (Object.keys(data).length === 0) {
    return NextResponse.json(
      { error: 'At least one of max_lr, gaussian_mean, gaussian_std, weight_decay, content_or_style, timestep_type must be provided' },
      { status: 400 }
    );
  }
