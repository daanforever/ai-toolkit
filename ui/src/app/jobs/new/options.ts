...
export const modelArchs: ModelArch[] = [
  ...
  },
  {
    ...
    defaults: {
      ...
      'config.process[0].train.timestep_weighting': ['weighted', 'none'],
    },
    disableSections: ['network.conv'],
  ...
  {
    name: 'zimage_diffsynth',
    label: 'Z-Image DiffSynth',
    group: 'image',
    defaults: {
      // default updates when [selected, unselected] in the UI (same as Z-Image base)
      'config.process[0].model.name_or_path': ['Tongyi-MAI/Z-Image', defaultNameOrPath],
      'config.process[0].model.quantize': [true, false],
      'config.process[0].model.quantize_te': [true, false],
      'config.process[0].model.low_vram': [true, false],
